	...
	for( pdx = 0; pdx < model->params->n_total; pdx++ )
	{
		...
		while(infection_event != NULL)
		{
			if( time_infected_infection_event(infection_event) != UNKNOWN )
			{
				...
				strain_idx[ idx ] = infection_event->strain_idx;
...
void write_transmissions( model *model )
{
	...
	for( pdx = 0; pdx < model->params->n_total; pdx++ )
	{
		...
		{
			if( time_infected_infection_event(infection_event) != UNKNOWN )
				fprintf(output_file ,"%li,%i,%li,%i,%i,%i,%i,%i,%i,%li,%i,%li,%i,%i,%i,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.3f\n",
					indiv->idx,
					indiv->age_group,
					indiv->house_no,
					indiv->occupation_network,
